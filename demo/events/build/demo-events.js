...
Main.run = function() {
	var container = dtx.Tools.find("#mainarea");
	var _g = 0;
	while(_g < 10) {
		var i = _g++;
		var b = new Button();
		dtx.collection.DOMManipulation.appendTo(b,null,container);
		dtx.collection.ElementManipulation.setText(b,"Button " + i);
	}
}
var dtx = {}
...
Button.__super__ = dtx.widget.Widget;
Button.prototype = $extend(dtx.widget.Widget.prototype,{
	get_template: function() {
		return "<button class=\"btn btn-danger\">My Button</button>";
	}
	,__class__: Button
});
...
Array.__name__ = true;
Button.__meta__ = { obj : { template : ["<button class='btn btn-danger'>My Button</button>"]}};
